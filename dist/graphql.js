...
exports.getGPRankList = exports.report = exports.getCreatStrategyinfo = exports.getPoolHourPrices = exports.performance = exports.riskManagement = exports.getDayTvl = exports.getPoolPrice = exports.getTokenList = exports.calculatetoken0token1 = exports.strategyEntities = exports.getSingleStrategy = exports.getPositionInfo2 = exports.getPositionInfo = exports.getinvestList = void 0;
...
async function getinvestList() {
    const query = `
      ...
      `;
    return node_fetch_1.default(graphql, {
        method: "post",
        headers: {
...
async function getSingleStrategy(sid) {
    ...
    const query = `
  {
    ...
    `;
    return node_fetch_1.default(lib_config_1.ContractAddress[lib_config_1.userInfo.chainID].strateggql, {
        method: "post",
        headers: {
            "Content-type": "application/json",
        },
        ...
        if (data.data.strategyEntities.length > 0) {
            ...
            return {
                data: Object.assign({ sid: strategyEntitie.sid, pool: strategyEntitie.pool, token0: strategyEntitie.token0.symbol, token1: strategyEntitie.token1.symbol, token0Price: res.token0Price, createdAtTimestamp: strategyEntitie.createdAtTimestamp, currLiquidity: strategyEntitie.currLiquidity, currPriceLower: currPriceLower, currPriceUpper: currPriceUpper, fee0: fee0, fee1: fee1, accFee0: strategyEntitie.accFee0, accFee1: strategyEntitie.accFee1, collectFee0: result.data.fee0, collectFee1: result.data.fee1, accumulativefee: (+fee0 + +fee1 * +res.token0Price), unbalance0: +token0token1Info.token0amount - strategyEntitie.preInvest0, unbalance1: +token0token1Info.token1amount - strategyEntitie.preInvest1 }, token0token1Info)
            };
        }
        else {
            return { data: {} };
 ...
 * @returns
 */
async function strategyEntities() {
    let res = await getPoolPrice();
    const query = `
  {
    strategyEntities(where: {user: "${lib_config_1.userInfo.account}",end:false}) {
      sid
      pool
      token0 {
        symbol
    ...
    return node_fetch_1.default(lib_config_1.ContractAddress[lib_config_1.userInfo.chainID].strateggql, {
        ...
        return strategyEntities.map((item) => {
            ...
            return Object.assign({ sid: item.sid, pool: item.pool, token0: item.token0.symbol, token1: item.token1.symbol, token0Price: res.token0Price, accFee0: item.accFee0, accFee1: item.accFee1, createdAtTimestamp: item.createdAtTimestamp, currLiquidity: item.currLiquidity, currPriceLower: currPriceLower, currPriceUpper: currPriceUpper, accumulativefee: +item.accFee0 + +item.accFee1 * +res.token0Price, unbalance0: +token0token1Info.token0amount - item.preInvest0, unbalance1: +token0token1Info.token1amount - item.preInvest1 }, token0token1Info);
        });
    }).then(async (data) => {
        ...
        await sids.reduce(async (pre, sid, i) => {
            await pre;
            let result = await index_1.collect(sid);
...
async function getGPRankList() {
    const query = `
  {
    ...
    `;
    return node_fetch_1.default(lib_config_1.ContractAddress[lib_config_1.userInfo.chainID].rankgql, {
        method: "post",
        headers: {
            "Content-type": "application/json",
        },
